#!/bin/bash
# Ce sript stoppe le service JBoss pour l'ensemble d'un environnement
PRG=`dirname $0`
...
confFile=$CURRENT_DIR/../conf/$env/polo_initd.conf
...
stopComponent ()
{
   ...
   then
     ...
     ssh $ADMIN_USER@$appHost sudo /etc/init.d/$2 stop
#     ssh $ADMIN_USER@$appHost  /etc/init.d/$2 stop
} 
...
do
   ...
   if [ "$isDone" = "" ]
   then
#     stopComponent $appHost jboss-"$env"pgw
     stopComponent $appHost jboss-$jbInst

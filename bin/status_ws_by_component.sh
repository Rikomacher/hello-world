...
# Ce sript affiche le status des services pour un ou plusieurs composants WS
PRG=`dirname $0`
...
{
    echo
    echo "Ce sript affiche le status des services pour un ou plusieurs composants WS"
...
# fonction status d'une instance
statusComponent ()
{
   if [ -z "$1" ]
   then
   ...
   then
     ...
     ssh -tT $ADMIN_USER@$appHost sudo systemctl status jboss-$jbInst
     echo ""
}
